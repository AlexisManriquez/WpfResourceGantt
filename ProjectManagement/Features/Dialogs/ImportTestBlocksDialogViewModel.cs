    ...
    public class ImportTestBlocksDialogViewModel : ViewModelBase
    {
        ...
        private async Task ImportAsync()
        {
            ...
            try
            {
                ...
                string gateSystemId = SelectedGate.Id.Contains("|") ? SelectedGate.Id.Split('|')[0] : SelectedGate.Id;
                _dataService.MarkSystemDirty(gateSystemId);
        ...
        private string ConvertExcelToCsv(string excelFilePath)
        {
            ...
            // --- NEW LOGIC: Exclude blank rows generated by Excel ---
            // Make sure this is outside the 'finally' block so Excel is done with the file!
            ...
            return tempCsv;
#else
    throw new NotSupportedException("Excel import is disabled in this build configuration.");
#endif
        }
        private List<ProgressBlock> ParseCsv(string path)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(line.Replace(",", "").Replace("\"", "")))
                    continue;
